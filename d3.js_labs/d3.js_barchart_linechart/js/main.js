...
const createLineChart = (data) => {
  ...
  /* Define x-axis, y-axis, and color scales */
  const yScale=d3.scaleLinear().domain([0,d3.max(data,d=>d.value)]).range([margin.left, width - margin.right]);

	const xScale=d3.scaleTime().domain(d3.extent(data,d=>d.date)).range([height - margin.bottom, margin.top]);
  ...
  /* Create line paths for each country */
	...
	const path=svg.selectAll('path').data(group).join('path').attr('d',([i,d])=>line(d)).style('stroke','lightgrey').style('stroke_width',2).style('fill','transparent');
